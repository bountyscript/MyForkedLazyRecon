...
getwaybackurl(){
  ...
  < $TARGETDIR/enumerated-subdomains.txt waybackurls | sort -u | grep 's3.amazonaws\|s3.us-east-2.amazonaws.com\|s3.us-east-1.amazonaws.com\|s3.us-west-1.amazonaws.com\|s3.us-west-2.amazonaws.com\|s3.af-south-1.amazonaws.com\|s3.ap-east-1.amazonaws.com\|s3.ap-south-1.amazonaws.com\|s3.ap-northeast-3.amazonaws.com\|s3.ap-northeast-2.amazonaws.com\|s3.ap-southeast-1.amazonaws.com\|s3.ap-southeast-2.amazonaws.com\|s3.ap-northeast-1.amazonaws.com\|s3.ca-central-1.amazonaws.com\|s3.cn-northwest-1.amazonaws.com.cn\|s3.eu-central-1.amazonaws.com\|s3.eu-west-1.amazonaws.com\|s3.eu-west-2.amazonaws.com\|s3.eu-south-1.amazonaws.com\|s3.eu-west-3.amazonaws.com\|s3.eu-north-1.amazonaws.com\|s3.me-south-1.amazonaws.com\|s3.sa-east-1.amazonaws.com'  > $TARGETDIR/tmp/s3_waybackurls_output.txt
...
main(){
  ...
  if [[ -n "$mad" ]]; then
    ...
    touch $rawList
  #if [[ -n "$fuzz" || -n "$brute" ]]; then
    if [ ! -d $TARGETDIR/ffuf/ ]; then 
      mkdir $TARGETDIR/ffuf/ 
    fi
    if [ ! -d $TARGETDIR/gospider/ ]; then 
      mkdir $TARGETDIR/gospider/ 
    fi
    if [ ! -d $TARGETDIR/page-fetched/ ]; then 
      mkdir $TARGETDIR/page-fetched/ 
    fi
    # to work with gf ssrf output
    if [ ! -f $TARGETDIR/page-fetched/pagefetcher_output.txt ]; then 
